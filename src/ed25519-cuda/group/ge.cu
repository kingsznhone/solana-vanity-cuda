...
// Direct indexing is faster for offline vanity search, but is not constant-time.
static void __host__ __device__ select(ge_precomp *t, int pos, signed char b)
{
    ...
    const ge_precomp *selected = &base[pos][index - 1];
    if (b < 0)
    {
        fe_copy(t->yplusx, selected->yminusx);
        fe_copy(t->yminusx, selected->yplusx);
        fe_neg(t->xy2d, selected->xy2d);
    }
    else
    {
        fe_copy(t->yplusx, selected->yplusx);
        fe_copy(t->yminusx, selected->yminusx);
        fe_copy(t->xy2d, selected->xy2d);
    }
}
...
/*
h = a * B using T[position][value] = value * 2^(16*position) * B.

This device-only implementation is for Ed25519 private scalars after clamping.
It is not a replacement for reduced-scalar signing and verification paths.
*/
void __device__ ge_scalarmult_base_fixed16_clamped(ge_p3 *h, const unsigned char *a,
                                                   const ge_precomp *table)
{
    ge_p1p1 sum;
    ge_p3_0(h);
    for (unsigned int position = 0; position < 16; ++position)
    {
        unsigned int value = static_cast<unsigned int>(a[2 * position]) |
                             (static_cast<unsigned int>(a[2 * position + 1]) << 8);
        if (value == 0)
        {
            continue;
        }
        const ge_precomp *selected = &table[(position << 16) | value];
        ge_madd(&sum, h, selected);
        ge_p1p1_to_p3(h, &sum);
    }
}
