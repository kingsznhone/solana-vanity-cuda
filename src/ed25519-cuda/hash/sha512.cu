...
#ifdef __CUDA_ARCH__
#define K_DEF __device__ __constant__
#else
#define K_DEF
#endif
...
void __device__ __host__ sha512_32_first_half(const unsigned char *message, unsigned char *out)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        LOAD64H(schedule[i], message + (8 * i));
    }
    schedule[4] = UINT64_C(0x8000000000000000);
    for (int i = 5; i < 15; ++i)
    {
        schedule[i] = 0;
    }
    schedule[15] = UINT64_C(256);
...
#pragma unroll 80
    for (int i = 0; i < 80; ++i)
    {
        ...
        if (i >= 16)
        {
            schedule[index] += Gamma1(schedule[(i - 2) & 15]) + schedule[(i - 7) & 15] + Gamma0(schedule[(i - 15) & 15]);
        }
        uint64_t t0 = h + Sigma1(e) + Ch(e, f, g) + K[i] + schedule[index];
